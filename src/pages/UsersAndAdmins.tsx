...
export default function UsersAndAdminsPage() {
    const { user, allUsers, totalUsers, fetchUsers, companies, fetchCompanies } = useAuthStore();
    ...
    }, [searchTerm, statusFilter, roleFilter, companyFilter, page]);

    useEffect(() => {
        fetchCompanies({ limit: 100 });
    }, [fetchCompanies]);

    if (user?.userRole !== "SUPER_ADMIN" && user?.userRole !== "MODERATOR") {
        ...
        <DashboardLayout>
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight text-gray-900 ml-4">Users & Admins</h2>
                        <p className="text-muted-foreground ml-4">
                            Manage system administrators and company level users.
                        </p>
                    </div>
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        className="flex h-9 w-[160px] rounded-md border border-input bg-muted/50 px-3 py-1 text-xs shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                    >
                            ...
                            <table className="w-full text-left text-sm">
                                <thead className="bg-muted/50 border-y text-muted-foreground font-medium">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 hidden lg:table-cell">Company</th>
                                    ...
                                    {isLoading && allUsers.length === 0 ? (
                                        Array.from({ length: 5 }).map((_, i) => (
                                            <tr key={i} className="animate-pulse">
                                                ...
                                                <td className="px-6 py-4"><div className="h-6 w-32 bg-muted rounded-full" /></td>
                                        ...
                                        <tr>
                                            <td colSpan={5} className="px-6 py-20 text-center text-muted-foreground">
                                                <div className="flex flex-col items-center gap-3">
                                                    ...
                                                    <p>{isLoading ? "Updating results..." : "No users found matching your filters."}</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                                ...
                                                <tr key={u.id} className={cn(
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <div className="font-semibold text-gray-900 line-clamp-1">{u.name || "N/A"}</div>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col gap-1.5">
                                                            <div className={cn(
                                                                "inline-flex items-center w-fit px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wider",
                                                                getRoleStyles(u.userRole)
                                                            )}>
                                                                <Shield className="h-3 w-3 mr-1" />
                                                                {u.userRole.replace('_', ' ')}
                                                            </div>
                                                            <div className={cn(
                                                                "inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold border transition-colors w-fit",
