...
import { useAuthStore, type BalanceSheetResponse, type BalanceSheetSection } from "@/store/useAuthStore";
...
export default function BalanceSheet() {
    ...
    const handleExportPDF = () => {
        ...
        sections.forEach(section => {
            ...
            rows.push(["Total " + section.title, formatCurrencyForExport(section.data.total, currency)]);
    ...
    const handleExportExcel = () => {
        ...
        sections.forEach(section => {
            ...
            wsData.push([`Total ${section.title}`, formatCurrencyForExport(section.data.total, currency)]);
            wsData.push([]); // Spacer
        });

        // Grand Totals check (Assets = Liabilities + Equity)
        wsData.push(["Accounting Equation Check"]);
        wsData.push(["Total Assets", formatCurrencyForExport(data.totals.assets, currency)]);
        wsData.push(["Total Liab + Equity", formatCurrencyForExport(data.totals.liabilities + data.totals.equity, currency)]);
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Statement of financial position including Assets, Liabilities, and Equity.
                        </p>
                    </div>
                    <div className="flex gap-2">
                        ...
                        <Button variant="outline" onClick={handleExportExcel} disabled={!data || isLoading}>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className={cn(
                            ...
                            <div className="text-sm">
                                Assets ({formatCurrency(data.totals.assets, user?.company?.currency)}) =
                                Liabilities ({formatCurrency(data.totals.liabilities, user?.company?.currency)}) +
                                Equity ({formatCurrency(data.totals.equity, user?.company?.currency)})
                            </div>
                        </div>
                        ...
                        <Card className="bg-muted/50">
                            <CardContent className="pt-6">
                                <div className="flex justify-between items-center text-lg font-bold">
                                    ...
                                    <span>{formatCurrency(data.totals.assets, user?.company?.currency)}</span>
                                </div>
                                <div className="flex justify-between items-center text-lg font-bold mt-2">
                                    <span>Total Liabilities & Equity</span>
                                    <span>{formatCurrency(data.totals.liabilities + data.totals.equity, user?.company?.currency)}</span>
                                </div>
                            </CardContent>
                        </Card>
                    </div>
